...
typedef struct
{
  ...
  steppersw_config_t config;
  steppersw_config_t user_config;
...
static inline void stepper_setup_timer(stepper_state_t *state, steppersw_config_t *config, const stepper_info_t *io)
{
  float period = 0;
  ...
  {
    //DEBUG_PRINT("stepper_setup_timer: stopped, disabling timer\n");
...
#if 0
  DEBUG_PRINT("stepper_setup_timer:\n"
...
{
  uint8_t i;
  DEBUG_PRINT("SW stepper driver init\n");
  ...
  for (i = 0; i < MIN(n, STEPPER_MAX); i++)
  {
    ...
    g_stepper[i].sem = xSemaphoreCreateBinary();
    xSemaphoreTake(g_stepper[i].sem, portMAX_DELAY);
    //DEBUG_PRINT("Stepper driver %i initialized\n", i);
  }
...
{
  steppersw_config_t *config = &g_stepper[index].config;
  ...
  if (*mailbox == 1)
  {
    steppersw_config_t *user_config = &g_stepper[index].user_config;
...
#if 0
    DEBUG_PRINT("stepper_tick: @MAILBOX:\n"
      ...
      && (state->velocity != config->tvelocity))
  {
        ...
        || ((accel_sign < 0) && (state->velocity <= config->tvelocity)))
    {
      // velocity reached
      state->velocity = config->tvelocity;
      //DEBUG_PRINT("stepper id %i at target velocity of %i sps\n", index, (int32_t)config->tvelocity);
    }
...
int8_t stepper_stop(uint8_t index, uint8_t hard_stop)
{
  ...
  DEBUG_PRINT("stepper_stop: index %i\n", index);
  return (STEPPER_OK);
}
...
int8_t stepper_idle(uint8_t index)
{
  ...
  DEBUG_PRINT("stepper_idle: index %i\n", index);
  return (STEPPER_OK);
}
...
int8_t stepper_status(uint8_t index)
{
  ...
  DEBUG_PRINT("stepper_status: velocity %i SPS, target velocity %i SPS, delay %i, hwtimer %i, accel %i, steps %i, step_count %i, tick_count %i, mailbox %i\n",
...
int8_t stepper_waitfor(uint8_t index)
{
  ...
  {
    DEBUG_PRINT("stepper_waitfor: waiting for stepper %i\n", index);
    xSemaphoreTake(sem, portMAX_DELAY);
  }
  else
  {
    DEBUG_PRINT("stepper_waitfor: NOT waiting for stepper %i\n", index);
